...
App.Router.map(function() {
    this.resource('books');
    this.resource('book', { path: 'book/:id' })
    this.resource('authors');
    this.resource('genre', {path: 'genre/:genre'});
    this.resource('author', { path: 'author/:id' });
});
...
App.BookRoute = Ember.Route.extend({
    model: function(params) {
        return books.books.find(function(book) {
...
App.AuthorRoute = Ember.Route.extend({
    model: function(params) {
        return books.authors.find(function(author) {
...
App.GenreRoute = Ember.Route.extend({
    model: function(params) {
        return books.books.filter(function(book) {
            return book.genre == params.genre;
        });
    }
});

App.AuthorsController = Ember.ObjectController.extend({
    isExpanded: false,

    actions: {
        toogle: function(event) {
            Ember.Logger.debug('log value of foo:', event, this);
